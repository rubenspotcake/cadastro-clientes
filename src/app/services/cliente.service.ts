...
export class ClienteService {
  ...
  salvar(cliente: Cliente) {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(storageClientes)
    );
  }
  ...
  obterPorNome(nome: string): Cliente[] {
    ...
    if (nome && nome.trim().length > 0) {
      return todos.filter(
        (c) => c.nome && c.nome.toLowerCase().includes(nome.toLowerCase())
      );
    }
    return todos;
  }
  ...
  excluir(id: string): void {
    const todos: Cliente[] = this.obteterTodos();
    const novos = todos.filter((c) => c && c.id !== id);
    localStorage.setItem(ClienteService.REPO_CLIENTES, JSON.stringify(novos));
  }

  atualizar(cliente: Cliente): void {
    const todos: Cliente[] = this.obteterTodos();
    const index = todos.findIndex((c) => c && c.id === cliente.id);
    if (index !== -1) {
      todos[index] = cliente;
      localStorage.setItem(ClienteService.REPO_CLIENTES, JSON.stringify(todos));
    }
  }
}
